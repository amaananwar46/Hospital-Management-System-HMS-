...
export default function PurchaseHistory() {
  ...
  const filteredPurchases = filterPatient
    ? purchaseHistory.filter(
        (p) => String(p.patientId) === String(filterPatient)
      )
    : purchaseHistory;
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <p><b>Patient:</b> ${purchase.patientName}</p>
          <p><b>Date:</b> ${purchase.date} ${purchase.time}</p>
          ...
          <h3>Total Amount : ₹${purchase.totalAmount}</h3>
        ...
        {purchaseHistory.length > 0 && (
          <div className="filter-section">
            ...
            >
              ...
              {[...new Map(
                purchaseHistory.map((p) => [p.patientId, p])
              ).values()].map((p) => (
                <option key={p.patientId} value={p.patientId}>
                  {p.patientName}
                </option>
